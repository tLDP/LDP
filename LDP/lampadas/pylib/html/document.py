...
"""
Generates HTML output for a document.
"""
...
class DocumentView:
    ...
    def get_html(self, uri):
        if self.document.pub_status_code=='': pub_status_name = ''
        else: pub_status_name = self.document.pub_status.name[uri.lang]
        if self.document.type_code=='': type_name = ''
        else: type_name = self.document.type.name[uri.lang]
        if self.document.review_status_code=='': review_status_name = ''
        else: review_status_name = self.document.review_status.name[uri.lang]
        if self.document.tech_review_status_code=='': tech_review_status_name = ''
        else: tech_review_status_name = self.document.tech_review_status.name[uri.lang]
        if self.document.format_code=='': format_name = ''
        else: format_name = self.document.format.name[uri.lang]
        if self.document.dtd_code=='': dtd_name = ''
        else: dtd_name = self.document.dtd.name[uri.lang]
        if self.document.lang=='': language_name = ''
        else: language_name = self.document.language.name[uri.lang]
        if self.document.license_code=='': license_name = ''
        else: license_name = self.document.license.name[uri.lang]
