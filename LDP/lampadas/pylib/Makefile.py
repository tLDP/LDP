...
class Makefile:
	...
	def write_all(self):
		self.Lint.L.Log(3, 'Writing Makefile for all documents')
		for dockey in self.Lint.L.Docs.keys():
			self.write_doc(dockey)
		self.write_main_makefile()

	def write_doc(self, DocID):
		self.Lint.L.Log.Write(3, 'Writing Makefile for document ' + str(DocID))
		...
		self.Lint.L.Log(3, 'Writing Makefile for document ' + str(DocID) + ' complete.')
	...
	def write_makefile(self, doc, dir):
		...
		for file in doc.Files.keys():
			...
			if File.is_primary:
				...
				Makefile = Makefile + "BUILD_HTML = xsltproc --param quiet 1 --maxdepth 100 --nonet --novalid " + self.C.xslt_html + ' ' + xmlfile + ' > ' + htmlfile + " 2>>xsltproc.log\n"
				Makefile = Makefile + "BUILD_INDEX = xsltproc --param quiet 1 --maxdepth 100 --nonet --novalid " + self.C.xslt_chunk + ' ' + xmlfile + " 2>>xsltproc.log\n"
	...
	def write_main_makefile(self):
		...
		rebuildmake = ''
		makeneeded = 0
		for docid in self.Lint.L.Docs.keys():
			Doc = self.Lint.L.Docs[docid]
			for file in Doc.Files.keys():
				File = Doc.Files[file]
				if File.is_primary:
		...
		if makeneeded:
			...
			fh = open(self.C.cache_dir + 'Makefile', 'w')
			fh.write(Makefile)
			fh.close
